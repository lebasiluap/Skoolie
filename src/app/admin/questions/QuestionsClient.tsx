...
interface Props {
  ...
  perPage: number
  topics: string[]
  filters: { topic?: string; type?: string; subtopic?: string; q?: string }
}
...
export default function QuestionsClient({ questions, total, page, perPage, topics, filters }: Props) {
  ...
  function applySearch() { router.push(buildUrl({ q: search, page: '1' })) }
  ...
  return (
    <div style={{ padding: 'clamp(20px,3vw,36px) clamp(16px,3vw,32px)', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 22 }}>
        <select value={filters.topic ?? ''} onChange={e => router.push(buildUrl({ topic: e.target.value || undefined, page: '1' }))} style={{ ...selectStyle, minWidth: 150 }}>
          <option value="">All topics</option>
          {topics.map(t => <option key={t} value={t}>{t}</option>)}
        </select>

        <select value={filters.type ?? ''} onChange={e => router.push(buildUrl({ type: e.target.value || undefined, page: '1' }))} style={selectStyle}>
        ...
        <div style={{ display: 'flex', gap: 8, flex: 1, minWidth: 220 }}>
          <input
            value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && applySearch()}
            placeholder="Search question text…" style={{ ...inputStyle, flex: 1 }} className="admin-input-focus"
          />
          <button onClick={applySearch}
            style={{ padding: '9px 18px', background: 'var(--teal)', color: 'var(--on-teal)', border: 'none', borderRadius: 12, fontSize: 13.5, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
            Search
          </button>
          {(filters.topic || filters.type || filters.q) && (
            <button onClick={() => router.push('/admin/questions')}
              style={{ padding: '9px 14px', background: 'var(--surface-3)', color: 'var(--text-soft)', border: 'none', borderRadius: 12, fontSize: 13.5, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
              Clear
            </button>
          )}
        </div>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 24, boxShadow: 'var(--shadow-lg)', padding: 28, width: '100%', maxWidth: 440 }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button onClick={() => setConfirmDeleteId(null)}
                style={{ padding: '10px 18px', border: 'none', background: 'transparent', color: 'var(--text-soft)', fontSize: 13.5, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                Cancel
              </button>
              <button onClick={confirmDelete}
                style={{ padding: '10px 20px', border: 'none', borderRadius: 12, background: 'var(--red)', color: '#fff', fontSize: 13.5, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>
                Yes, delete
              </button>
            </div>
          </div>
      ...
      {editingId && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', backdropFilter: 'blur(6px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 24, boxShadow: 'var(--shadow-lg)', padding: 28, width: '100%', maxWidth: 500 }}>
            <h3 style={{ margin: '0 0 3px', fontSize: 16, fontWeight: 800, color: 'var(--text)', textTransform: 'capitalize' }}>Edit {editField}</h3>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button onClick={saveEdit} disabled={saving}
                style={{ padding: '10px 24px', border: 'none', borderRadius: 12, background: 'var(--teal)', color: 'var(--on-teal)', fontSize: 13.5, fontWeight: 800, cursor: saving ? 'default' : 'pointer', fontFamily: 'inherit', opacity: saving ? 0.6 : 1 }}>
                {saving ? 'Saving…' : 'Save'}
              </button>
            </div>
          </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
              {['Topic', 'Subtopic', 'Type', 'Difficulty', 'Question', 'Region', 'Access', 'Actions'].map(h => (
              ...
              return (
                <tr key={q.id} style={{ borderBottom: isLast ? 'none' : '1px solid var(--border)' }} className="admin-table-row">
                  <td style={{ padding: '10px 14px', whiteSpace: 'nowrap' }}>
                    <button onClick={() => startEdit(q.id, 'topic', q.topic)}
                      style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 13, fontWeight: 700, color: 'var(--text)', fontFamily: 'inherit', textAlign: 'left' }}
                      className="admin-cell-btn">
                      {q.topic}
                    </button>
                  </td>
                  <td style={{ padding: '10px 14px', whiteSpace: 'nowrap' }}>
                    <button onClick={() => startEdit(q.id, 'subtopic', q.subtopic ?? '')}
                      style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 12.5, color: 'var(--text-soft)', fontFamily: 'inherit', textAlign: 'left' }}
                      className="admin-cell-btn">
                      {q.subtopic ?? '—'}
                    </button>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <button onClick={() => startEdit(q.id, 'question_type', q.question_type)}
                      style={{ background: typeS.bg, color: typeS.color, border: 'none', borderRadius: 999, padding: '4px 10px', fontSize: 11.5, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
                      {q.question_type}
                    </button>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <button onClick={() => startEdit(q.id, 'difficulty', q.difficulty)}
                      style={{ background: diffS.bg, color: diffS.color, border: 'none', borderRadius: 999, padding: '4px 10px', fontSize: 11.5, fontWeight: 800, cursor: 'pointer', textTransform: 'capitalize', fontFamily: 'inherit' }}>
                      {q.difficulty}
                    </button>
                  </td>
                  <td style={{ padding: '10px 14px', maxWidth: 300 }}>
                    <button onClick={() => startEdit(q.id, 'question_text', q.question_text)}
                      style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 12.5, color: 'var(--text-soft)', fontFamily: 'inherit', textAlign: 'left', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}
                      className="admin-cell-btn">
                      {q.question_text}
                    </button>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <button onClick={() => startEdit(q.id, 'region', q.region ?? 'universal')}
                      style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 12.5, color: 'var(--text-soft)', fontFamily: 'inherit' }}
                      className="admin-cell-btn">
                      {q.region ?? '—'}
                    </button>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <button onClick={() => startEdit(q.id, 'access_key', q.access_key ?? '')}
                  ...
                  <td style={{ padding: '10px 14px' }}>
                    <button onClick={() => handleDelete(q.id, q.question_text)} disabled={deleting === q.id}
                      style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 12.5, fontWeight: 700, color: 'var(--red)', fontFamily: 'inherit', opacity: deleting === q.id ? 0.4 : 1 }}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {page > 1 && (
            <a href={buildUrl({ page: String(page - 1) })}
              style={{ padding: '8px 18px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, color: 'var(--text-soft)', fontSize: 13.5, fontWeight: 700, textDecoration: 'none' }}
