// All countries (ISO 3166-1), flags derived from the ISO code.
// user_profiles.country drives content visibility server-side (region_visible):
// everyone sees 'universal' questions; country-specific questions (e.g. region
// 'ghana') only show for users based in that country.

const flagOf = (iso: string) =>
  iso.toUpperCase().replace(/./g, c => String.fromCodePoint(127397 + c.charCodeAt(0)))

/** Shown first in the picker before any search. */
export const POPULAR_COUNTRIES = ['Ghana', 'Nigeria', 'Kenya', 'South Africa', 'United Kingdom', 'United States']
...
export const COUNTRIES: { name: string; flag: string }[] =
  RAW.map(([code, name]) => ({ name, flag: flagOf(code) }))
